...
public class RaceConditionToplamAtomic {
    private static final AtomicLong toplam = new AtomicLong(0);
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            threadler[i] = new Thread(() -> {
                ...
                for (long j = baslangic; j <= bitis; j++) {
                    toplam.addAndGet(j); // atomik ekleme
                }
            });
            threadler[i].start();
        }
        ...
        System.out.println("Sonuç: " + toplam.get() + " | Süre: " + sure + "ms");
    }
}
